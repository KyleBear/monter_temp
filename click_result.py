...
#     # curl/wget 설치 여부 확인
#     logger.info("휴대폰에서 curl/wget 설치 여부 확인 중...")
#     curl_available = False
#     wget_available = False
    
#     try:
#         curl_check = adb.run_command('shell', 'which', 'curl', timeout=3)
#         if curl_check.returncode == 0 and curl_check.stdout.strip():
#             curl_available = True
#             logger.info(f"✓ curl 사용 가능: {curl_check.stdout.strip()}")
#     except:
#         pass
    
#     try:
#         wget_check = adb.run_command('shell', 'which', 'wget', timeout=3)
#         if wget_check.returncode == 0 and wget_check.stdout.strip():
#             wget_available = True
#             logger.info(f"✓ wget 사용 가능: {wget_check.stdout.strip()}")
#     except:
#         pass
    
#     if not curl_available and not wget_available:
#         logger.warning("⚠ 휴대폰에 curl과 wget이 모두 설치되어 있지 않습니다.")
#         logger.warning("다음 방법을 시도합니다:")
#         logger.warning("1. Android의 내장 명령어 사용")
#         logger.warning("2. Python requests를 통한 간접 확인")
        
#         # 대안 1: Android 내장 명령어로 시도 (일부 기기에서만 작동)
#         try:
#             # am start를 사용하여 브라우저로 IP 확인 페이지 열기 (복잡함)
#             # 대신 간단하게 로컬 IP만 확인
#             result = adb.run_command('shell', 'getprop', 'net.dns1', timeout=3)
#             if result.returncode == 0:
#                 logger.info(f"DNS 서버: {result.stdout.strip()}")
#         except:
#             pass
        
#         # 대안 2: Python requests를 사용 (PC에서 확인하지만, 휴대폰 네트워크를 통해)
#         # 이 방법은 실제로는 PC의 IP를 가져오므로 정확하지 않음
#         logger.warning("⚠ 외부 IP 확인을 위해 curl 또는 wget이 필요합니다.")
#         logger.warning("해결 방법:")
#         logger.warning("1. 휴대폰에 curl 설치 (Root 필요): adb shell su -c 'busybox wget -O /system/bin/curl ...'")
#         logger.warning("2. 또는 휴대폰 브라우저에서 직접 확인: https://api.ipify.org")
#         return None
